...
# Geo Files
czFile       = "geos/usa_county_cz_zip_5.json"
...
def generatePathsFileFromTemplate(args):
	...
	if(args.zoomToRegion):
		czFile = "geos/usa_county_cz_zip_5.json" # HACK! TODO: Fix this
	else:
		czFile = re.sub(r"[\\\/]+", "/", czFile)	

	# use special files for original county or CZ maps
	...
	# use special files for high-res county maps
	if (args.geoType)=="county2010_ne":
		czFile = "geos/county2010_ne.json"
		
	# use special files for tract2010_wa (Washington state tract-level maps)
	if (args.geoType)=="tract2010_wa":
		czFile = "geos/tract2010_wa.json"

	# use special files for tract2010_um (Upper midwest - IL, IA, WI, MN - tract-level maps)
	if (args.geoType)=="tract2010_um":
		czFile = "geos/tract2010_um.json"
		
	# use special files for tract2010_lores (national tract-levels maps, not removing water bodies)
	if (args.geoType)=="tract2010_lores":
		czFile = "geos/tract2010_lores.json"
	
	# definitely use special files for chicago_ca (Chicago community areas)
	if (args.geoType)=="chicago_ca":
		czFile = "geos/chicago_ca.json"

	# definitely use special files for counties_hr (High-res counties from aggregating national tracts shapefile)
	if (args.geoType)=="counties_hr":
		czFile = "geos/counties_hr.json"
		
	# definitely use special files for puma2010 (public-use microdata geography)
	if (args.geoType)=="puma2010":
		czFile = "geos/puma.json"
...
output_svg_filename = r"C:\Users\thedr\Dropbox\ado\mapmaker2\trunk\test.svg"
...
if (not args.noImage):
	...
	if(platform.startswith("win")):
		...
		if (args.makeSVG):
			retcode = subprocess.call([phantom_winpath, "phantom-crowbar.js", html_code_file, "choropleth", output_svg_filename])
	else:
		retcode = subprocess.call([phantom_macpath, rasterfile, html_code_file, output_filename])
		if (args.makeSVG):
			retcode = subprocess.call([phantom_macpath, "phantom-crowbar.js", html_code_file, "choropleth", output_svg_filename])
