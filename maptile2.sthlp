{smcl}
{* *! version 0.99  4oct2016}{...}
{vieweralsosee "maptile" "help maptile"}{...}
{viewerjumpto "Syntax" "maptile2##syntax"}{...}
{viewerjumpto "Description" "maptile2##description"}{...}
{viewerjumpto "Options" "maptile2##options"}{...}
{viewerjumpto "Examples" "maptile2##examples"}{...}
{title:Title}

{pstd}
{hi:maptile2} {hline 2} Scalable pretty vector-based U.S. chloropleths (heat maps)
...
{cmd:maptile2}
{varname} {ifin}{cmd:,}
 {cmdab:geo:graphy(}{it:{help maptile2##usegeo:geoname}}{cmd:)} [{it:options}]
...
{syntab :Bins}
{synopt :{opt nq:uantiles(#)}}number of bins, defined as quantiles; default is {bf:6}{p_end}
...
{synopt :{opt meshcounties}}draw county lines on tract-level geographies{p_end}
{synopt :{opt meshstates}}draw state lines on tract/county-level geographies{p_end}
...
{syntab :Advanced}
{synopt :{opt debug}}debug mode; does not close terminal window when creating map{p_end}
{synoptline}
...
{pstd}
{cmd:maptile2} generates publication-quality heat maps (chloropleths, for fancy-pants) of the United States using a combination of Stata, Python, D3JS, and PhantomScript.

{pstd}
{cmd:maptile2} can display maps of the United States at several levels of geographic detail: states, commuting zones, counties, five-digit ZIP codes (poorly), and census tracts.

{pstd}
{cmd:maptile2} requires Python (preferably 3.x) and Jimmy Narang's map-making scripts (for us, this is in Dropbox in the ado/mapmaker directory). 

Python must be set as a valid system environment variable; that is, the command 'python' in the command prompt should open up the Python interpreter. 

In addition, Dropbox/ado/ must be synced to your machine, and the global macro $dropbox should point to the Dropbox directory's path, and
...
If savegraph() is not specified at all, maptile2 writes to temp.png in the current working directory.
...
{phang2}. {stata maptile2 poor_share, geo(cz)}{p_end}

{pstd}Zoom in to a few states{p_end}
{phang2}. {stata maptile2 poor_share if inlist(statename,"New York", "Pennsylvania", "New Jersey", "Connecticut"), geo(cz)}{p_end}
...
{phang2}. {stata maptile2 rate, geo(county) geovar(county_id)}{p_end}

{pstd}County-level map, Washington state{p_end}
{phang2}. {stata maptile2 rate if stateabbrv=="WA", geo(county) geovar(county_id) zoom}{p_end}
